<script setup lang="ts">
// S3（缺陷#6 收敛）：预付款台账（prepayments 表为单一真源）。
// 行 = 台账行：登记日期 / 供应商 / 采购合同 / 设备SN（可空=手工预付未分摊）/ 总额 / 已结转 / 余额。
// 结转随每月按台计费自动发生（直线法），同扣台账与设备镜像。
import { h, onMounted, ref } from 'vue'
import { NCard, NDataTable, NTag, useMessage } from 'naive-ui'
import { api } from '../api/client'
import { errMsg } from '../utils/errMsg'
import { money } from '../utils/format'

const msg = useMessage()
const items = ref<any[]>([])
const loading = ref(false)

async function refresh() {
  loading.value = true
  try {
    const { data } = await api.get('/prepayments/summary')
    items.value = data.items
  } catch (e: any) { msg.error(errMsg(e)) }
  finally { loading.value = false }
}
onMounted(refresh)
</script>

<template>
  <div>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
      <h3>预付款台账</h3>
      <span class="muted tiny">以资金流水台账为准（含日期/供应商/合同）；结转随每月按台计费自动发生</span>
    </div>
    <n-card size="small">
      <n-data-table size="small" :bordered="false" striped :loading="loading" :pagination="{ pageSize: 15 }"
        :columns="[
          { title: '登记日期', key: 'payment_date', width: 110, render: (r: any) => r.payment_date || '待补' },
          { title: '供应商', key: 'supplier_name', render: (r: any) => r.supplier_name || '待补' },
          { title: '采购合同', key: 'contract_no', render: (r: any) => r.contract_no || '—' },
          { title: '设备SN', key: 'sn', render: (r: any) => r.sn || '—' },
          { title: '项目', key: 'project_name' },
          { title: '预付款总额', key: 'prepayment_amount', align: 'right' as const, render: (r: any) => money(r.prepayment_amount) },
          { title: '累计已结转', key: 'settled_amount', align: 'right' as const, render: (r: any) => money(r.settled_amount) },
          { title: '余额', key: 'remaining', align: 'right' as const, render: (r: any) => money(r.remaining) },
          { title: '状态', key: 'settled', width: 90, render: (r: any) =>
              h(NTag, { size: 'small', type: r.settled ? 'success' : 'warning', bordered: false },
                () => (r.settled ? '已结清' : '结转中')) },
        ]"
        :data="items"
        :row-key="(r: any) => r.id">
        <template #empty>暂无预付款记录（资金池「预付」登记或设备登记预付款后出现在这里）</template>
      </n-data-table>
    </n-card>
    <div class="muted tiny" style="margin-top:8px">
      口径：余额 = 预付款总额 − 累计已结转；月结转 = 总额 ÷ 合同月数（直线法，尾差末月收敛）；手工预付未分摊到设备时 SN 显示 —。
    </div>
  </div>
</template>

<style scoped>
.muted { color: #94A3B8; }
.tiny { font-size: 12px; }
</style>
