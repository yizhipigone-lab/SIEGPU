...
> **最后更新**：2026-08-13
> **最近里程碑**：二期收官 + 三期 §4.2 收入确认 + §4.3 对账中心（7 维）+ §4.4 退货链路完成——pytest 374 绿 / e2e 60 绿 / 浏览器真点 :8088；改动分类提交入库
...
# e2e（Playwright，当前 60 条；baseURL 走 :8088）
...
1. **三期 §4.5 预算管控 + 经营看板**：`project_budgets`（采购/预付款额度/费用/融资额度）+ 四类触发点校验（超预算预警可继续、超 10% 硬拦截走预算调整审批）+ Dashboard 升级（待办中心/核心指标/EBS 状态卡）。
2. **三期 §4.6 资金池预测引擎**：`cash_plan_items`（预测统一数据源）+ `fund_forecasts`（场景快照）——三期核心新增，建议单独排期。
3. **顺手待办**：「发 EBS 接口规范申请函」（外部流程，W1 起挂着）；`e2e/fetch-doubao.js` 来源待确认，勿盲目提交。

**接手者第一件事**：跟用户确认走哪条。**不要未经确认就开新实现**（规则1：先问清楚）。

> 工作区现状提示：二期 + 三期 §4.2/§4.3 已入库；§4.4 退货链路改动验收后提交。
...
> **文档版本**：2026-08-13（二期收官 + 三期 §4.2/§4.3/§4.4 版）｜**下一状态**：三期 §4.5 预算管控+经营看板 / §4.6 资金池预测（等用户拍板）
