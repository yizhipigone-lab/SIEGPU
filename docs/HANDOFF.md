...
> **最后更新**：2026-08-13
> **最近里程碑**：二期收官 + 三期 §4.2 收入确认 + §4.3 对账中心（7 维）+ §4.4 退货链路 + §4.5 经营看板完成——pytest 379 绿 / e2e 61 绿 / 浏览器真点 :8088；改动分类提交入库
...
# e2e（Playwright，当前 61 条；baseURL 走 :8088）
...
### 3.11 三期 §4.4 采购退货 + 合同终止资源释放（刚完成，✅ 已端到端验证）

**退货全链路**（父计划 §4.4）：退货申请（单台/批量，点亮设备禁退）→ 出库确认（设备置「已退货」，状态机入口有守门）→ 供应商收货（财务联动：已转固资产软删减少 + audit 留痕）→ 开红字发票（PAYABLE + reversal_of_id 挂原票）→ 退款登记（capital IN）→ 退款核销（payment_settlements 挂红票）。预付款追回额（Σ设备剩余预付款）创建时落字段。

- **新表 2 张**：`return_orders` / `return_order_devices`（迁移 0017）；devices.status CHECK 扩 `'已退货'`（downgrade 先清已退货行 guard）。
- **合同终止资源释放**：销售合同终止 → 设备摘下 sales_contract_id（回退可用），audit 记释放台数。
- **前端**：ReturnView（列表 + 新增〔项目→设备多选，点亮/已退不出现〕+ 详情抽屉逐步推进按钮）。
- **测试**：`test_return.py` 5 条（申请 golden/守卫/全链/强顺序/终止释放）；e2e `tests/returns.spec.ts` 1 条（UI 申请→4 步推进→全链 API 追值）。
- **验证证据**：pytest 374 全绿 / e2e 60 全绿 / 迁移 up→down→up 实测 / 浏览器真点（截图 `e2e/screenshots/p3-returns.png`）。

### 3.12 三期 §4.5 经营看板（刚完成，✅ 已端到端验证）

**Dashboard 升级**（cfo/管理员分支新增「经营看板」区，角色化首页与「待处理」卡零改动）：

- **核心指标 8 项**：当期合同额（本月新签）/累计回款/开票金额/确认收入/融资余额（已放款−已还本金）/资金池余额/监管账户余额/设备交付进度（点亮/总台数）。
- **待办中心**（计数+跳转）：付款/收入审批、待投保确认、预付款未结清设备、待还款（30 天）、监管账户预警（复用 dim4 口径）、资金缺口预警（复用 alert POOL_INSUFFICIENT）。
- **资金预测概览**（简易版，未接 §4.6 引擎）：未来 3 个月 流入=执行中销售合同月租，流出=当月到期还款+未付采购发票，期末滚动衔接，缺口行标红。
- **EBS 同步状态卡**：成功/失败计数 + 最近同步时间 + 跳转 EBS 监控。
- **关键文件**：`services/business_board_service.py`、`endpoints/dashboard.py`（`GET /api/dashboard/business`）、`views/Dashboard.vue`（`data-testid="business-board"`）。
- **测试**：`test_business_dashboard.py` 5 条（指标 golden/融资余额/待办计数/预测滚动/EBS 计数）；e2e `tests/business-dashboard.spec.ts`。
- **验证证据**：pytest 379 全绿 / e2e 61 全绿 / 浏览器真点（截图 `e2e/screenshots/p3-business-board.png`）。
...
1. **三期 §4.5 预算管控**（经营看板已完成）：`project_budgets`（采购/预付款额度/费用/融资额度）+ 四类触发点校验（超预算预警可继续、超 10% 硬拦截走预算调整审批）。
2. **三期 §4.6 资金池预测引擎**：`cash_plan_items`（预测统一数据源）+ `fund_forecasts`（场景快照）——三期核心新增，建议单独排期；落地后把看板「资金预测概览」简易版换成引擎输出。
3. **顺手待办**：「发 EBS 接口规范申请函」（外部流程，W1 起挂着）；`e2e/fetch-doubao.js` 来源待确认，勿盲目提交。

**接手者第一件事**：跟用户确认走哪条。**不要未经确认就开新实现**（规则1：先问清楚）。

> 工作区现状提示：二期 + 三期 §4.2/§4.3/§4.4 已入库；§4.5 经营看板改动验收后提交。
...
> **文档版本**：2026-08-13（二期收官 + 三期 §4.2~§4.5 看板版）｜**下一状态**：三期 §4.5 预算管控 / §4.6 资金池预测（等用户拍板）
